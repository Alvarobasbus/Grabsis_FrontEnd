...
@Component({
  selector: 'app-registrar-grabado',
  templateUrl: './registrar-grabado.component.html',
  styleUrls: ['./registrar-grabado.component.css']
})
export class RegistrarGrabadoComponent implements OnInit {
  ...
  setFechaActual(){
   ...
   confirmarAutopartes(det: Detalle){
    ...
    Swal.fire({
      ...
      if(result.isConfirmed){ 
        ...
        this.subscripcion.add(
          this.grabadoService.grabadoAutopartes(this.grabado).subscribe({
            next: (respuesta) =>{
              this.buscar()
            } ,
            error: () => {
